...
export const useProjectStore = create<ProjectStore>((set, get) => ({
  ...
  subscribeToProjects: () => {
    ...
    set({ isLoading: true });
    const projectsQuery = query(ref(db, 'projects'), orderByChild('ownerId'), equalTo(user.uid));
    ...
    const unsubscribe = onValue(projectsQuery, (snapshot) => {
      ...
      set({ projects: projectsList as Project[], isLoading: false });
    }, (error) => {
    ...
    return () => off(projectsQuery, 'value', unsubscribe);
  },
  ...
  subscribeToProject: (id: string) => {
    ...
    const projectMetaRef = ref(db, `projects/${id}`);
  ...
  updateProject: async (id: string, data: Partial<Project>) => {
    ...
    try {
      ...
      Object.keys(data).forEach(key => {
        updates[`projects/${id}/${key}`] = (data as any)[key];
      });
      updates[`projects/${id}/updatedAt`] = now;
