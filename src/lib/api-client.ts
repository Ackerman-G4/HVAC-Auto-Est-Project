...
export const projectsApi = {
  ...
  create: async (data: any) => {
    ...
    try {
      // 1. Create the project metadata in the projects list.
      // This satisfies the "projects" rule.
  ...
  delete: async (id: string, permanent = false) => {
    ...
    if (permanent) {
      ...
      await update(ref(db, `projects/${id}`), { status: 'deleted', updatedAt: new Date().toISOString() });
    }
