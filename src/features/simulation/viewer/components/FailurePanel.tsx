  ...
  return (
    <div className="space-y-6">
      <div className="panel-glass grid grid-cols-2 gap-5 rounded-md border border-border/70 bg-card p-5 md:grid-cols-3">
        ...
        <Field label="Duration" unit="seconds">
          {(f) => (
            <input
              {...f}
              className="w-full rounded-md border border-border bg-background px-3.5 py-2.5 text-sm"
      ...
      {scenario !== 'power_loss' && hvacUnits.length > 0 && (
        // A label over a group of toggle buttons, not over one control. fieldset
        // and legend are the element pair that actually names a group; a <label>
        // here pointed at nothing.
        <fieldset>
          <legend className="mb-2 block text-[11px] font-semibold font-display text-muted-foreground">Select failed units</legend>
