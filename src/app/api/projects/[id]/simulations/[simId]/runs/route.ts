...
import { getSimulationCase, listRunJobs } from '@/lib/firebase/simulation-cases-store';
import {
  dispatchEngineeringRun,
...
function dispatchRefusalResponse(refusal: DispatchRefusal): NextResponse {
  switch (refusal.reason) {
    ...
    case 'ENGINEERING_TIER_NOT_PROVISIONED':
      return errorResponse(
        503,
        'Engineering tier not provisioned',
        `The OpenFOAM cloud path is not configured. Missing: ${refusal.missing.join(', ')}. ` +
          'Use the Preview tier, or complete plan phases C1–C2.',
        'ENGINEERING_TIER_NOT_PROVISIONED',
      );
    case 'SOLVER_DISPATCH_FAILED':
      // 502, not 500: the fault is downstream of this service.
      return errorResponse(502, 'Dispatch failed', refusal.message, 'SOLVER_DISPATCH_FAILED');
  }
}
...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    const result = await dispatchEngineeringRun(productionDispatchDeps, {
      projectId,
      caseId: simId,
      user: auth.user,
    ...
    if (!result.ok) return dispatchRefusalResponse(result);

    // 202 Accepted — never block the request on the solve (plan §4.3).
    return NextResponse.json(
      { runId: result.runId, status: 'running', source: 'openfoam', execution: result.execution },
      { status: 202 },
    );
  } catch (error) {
