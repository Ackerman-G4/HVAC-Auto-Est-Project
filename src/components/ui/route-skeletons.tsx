  ...
  return (
    <div className="mb-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="rounded-lg border border-border/70 bg-card/80 p-5">
    ...
    <div className="overflow-hidden rounded-lg border border-border/70 bg-card/80">
      ...
      {Array.from({ length: rows }).map((_, i) => (
        <div key={i} className="flex items-center gap-6 border-b border-border/40 px-5 py-4 last:border-0">
          ...
          <Skeleton className="h-4 w-24" />
          <Skeleton className="ml-auto h-7 w-16 rounded-sm" />
        </div>
      ))}
    </div>
  );
...
export function WorkspaceSkeleton() {
  return (
    <div className="flex h-[calc(100vh-8rem)] gap-4">
      <div className="w-72 shrink-0 space-y-4 rounded-lg border border-border/70 bg-card/80 p-4">
        <Skeleton className="h-5 w-32" />
        {Array.from({ length: 6 }).map((_, i) => (
          <Skeleton key={i} className="h-10 w-full rounded-md" />
        ))}
      </div>
      <div className="flex flex-1 items-center justify-center rounded-lg border border-border/70 bg-card/60">
        <div className="text-center">
          <Skeleton className="mx-auto mb-3 h-12 w-12 rounded-lg" />
          <Skeleton className="mx-auto h-4 w-44" />
        </div>
      </div>
      <div className="w-80 shrink-0 space-y-4 rounded-lg border border-border/70 bg-card/80 p-4">
...
export function DetailSkeleton() {
  return (
    <div>
      <PageHeaderSkeleton />
      <div className="mb-8 flex gap-2 overflow-hidden rounded-lg border border-border/70 bg-card/80 p-3">
